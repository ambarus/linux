...
static int acpm_tmu_trip_temp_walk_cb(struct thermal_trip *trip, void *data)
{
	...
	twd->threshold[twd->i] = ((trip == twd->trip) ?
				  twd->temp / MCELSIUS : trip->temperature / MCELSIUS);
...
static int acpm_tmu_initialize_walk_cb(struct thermal_trip *trip, void *data)
{
	...
	twd->threshold[i] = trip->temperature / MCELSIUS;
	twd->hysteresis[i] = trip->hysteresis / MCELSIUS;
...
static int acpm_tmu_probe(struct platform_device *pdev)
{
	...
	priv->clk = devm_clk_get_prepared(dev, NULL);
	if (IS_ERR(priv->clk))
		return dev_err_probe(dev, PTR_ERR(priv->clk),
				     "Failed to get prepare the clock\n");

	irq = platform_get_irq(pdev, 0);
	if (irq < 0)
		return dev_err_probe(dev, irq, "Failed to get irq\n");

	ret = devm_request_threaded_irq(dev, irq, NULL, acpm_tmu_thread_fn,
	...
	if (ret)
		return dev_err_probe(dev, ret, "Failed to set runtime PM active\n");

	ret = devm_pm_runtime_enable(dev);
	if (ret) {
		clk_disable(priv->clk);
		return dev_err_probe(dev, ret, "Failed to runtime PM active enabled\n");
	}

	ret = devm_pm_runtime_get_noresume(dev);
	if (ret)
		return dev_err_probe(dev, ret, "Failed to get runtime PM noresume\n");

	ret = acpm_handle->ops.tmu.init(acpm_handle, priv->mbox_chan_id);
	if (ret)
		return dev_err_probe(dev, ret, "Failed to init TMU\n");
	...
	for (i = 0; i < priv->num_sensors; i++) {
		...
		if (IS_ERR(sensor->tzd))
			return dev_err_probe(dev, PTR_ERR(sensor->tzd),
					     "Failed to register sensor %d\n", i);
	}

	ret = acpm_tmu_init_sensors(priv);
	if (ret)
		return dev_err_probe(dev, ret, "Failed to init sensors\n");
	...
	if (ret)
		return dev_err_probe(dev, ret, "Failed to enable TMU\n");
...
static int acpm_tmu_resume(struct device *dev)
{
	...
	/* IPC with firmware while APB clock is likely still off/managed */
...
static const struct dev_pm_ops acpm_tmu_pm_ops = {
	...
	SET_RUNTIME_PM_OPS(acpm_tmu_runtime_suspend, acpm_tmu_runtime_resume, NULL)
};
...
MODULE_DESCRIPTION("Samsung Exynos GS TMU Driver");
MODULE_LICENSE("GPL");
